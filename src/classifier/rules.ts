...
const KEYWORD_RULES: Rule[] = [
  ...
  },
  {
    ...
    keywords: /(антисептик[а-яёa-z]*|дезинфицир[а-яёa-z]+|дезраствор[а-яёa-z]*|санитайзер[а-яёa-z]*|(?<![а-яёa-z])алмадез(?![а-яёa-z])|(?<![а-яёa-z])оптимакс(?![а-яёa-z])|(?<![а-яёa-z])аламинол(?![а-яёa-z]))/iu,
    confidence: 0.8,
  ...
  },
  {
    wave: "beauty-2025-w1",
    keywords:
      /((?<![а-яёa-z])мыло(?![а-яёa-z])|гел[а-яёa-z]+ для душа|пен[а-яёa-z]+ для ванн|соль для ванн|бомбочк[а-яёa-z]+ для ванн|средств[а-яёa-z]+ для стирки|для мытья посуды)/iu,
    confidence: 0.8,
  ...
  },
  {
    wave: "beauty-2025-w3",
    keywords:
      /(гель[- ]?лак[а-яёa-z]*|(?<![а-яёa-z])баз[а-яёa-z]+ (для|под) (гель|ногт)[а-яёa-z]*|камуфлирующ[а-яёa-z]+ баз[а-яёa-z]*|(?<![а-яёa-z])топ[а-яёa-z]* (для|без) (гель|липк)[а-яёa-z]*|праймер[а-яёa-z]*|дегидратор[а-яёa-z]*|бондер[а-яёa-z]*|(?<![а-яёa-z])лак[а-яёa-z]* для ногтей|полигел[а-яёa-z]*|акригел[а-яёa-z]*|акрил[а-яёa-z]+ (пудр[а-яёa-z]+|систем[а-яёa-z]+)|для наращивани[а-яёa-z]+|укреплени[а-яёa-z]+ ногтей|масл[а-яёa-z]+ для кутикул[а-яёa-z]*|ремувер[а-яёa-z]*|обезжириват[а-яёa-z]*|снятия (гель|лак)[а-яёa-z]*|крем[а-яёa-z]*|сыворотк[а-яёa-z]*|лосьон[а-яёa-z]*|тоник[а-яёa-z]*|мицеллярн[а-яёa-z]*|скраб[а-яёa-z]*|пилинг[а-яёa-z]*|маск[а-яёa-z]+ для (лица|рук|ног)|патч[а-яёa-z]*|парафин[а-яёa-z]*|пенк[а-яёa-z]+ для умывани[а-яёa-z]*|бальзам[а-яёa-z]* для губ|помад[а-яёa-z]*|блеск для губ|(?<![а-яёa-z])тушь(?![а-яёa-z])|(?<![а-яёa-z])тени(?![а-яёa-z])|пудр[а-яёa-z]*|румян[а-яёa-z]*|консилер[а-яёa-z]*|хайлайтер[а-яёa-z]*|тональн[а-яёa-z]*|(?<![а-яёa-z])тинт[а-яёa-z]*|карандаш для (глаз|губ|бровей)|зубн[а-яёa-z]+ паст[а-яёa-z]*|ополаскиват[а-яёa-z]* (для )?полости рта|солнцезащитн[а-яёa-z]*|\bspf\b)/iu,
    confidence: 0.75,
