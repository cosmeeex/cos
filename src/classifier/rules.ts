...
const KEYWORD_RULES: Rule[] = [
  ...
  },
  {
    wave: "hygiene-2026",
    keywords:
      /(зубн[а-яёa-z]+ (щетк|щётк|нит)[а-яёa-z]+|расческ[а-яёa-z]+|расчёск[а-яёa-z]+|гребен[а-яёa-z]+|пинцет[а-яёa-z]*|твизер[а-яёa-z]*|маникюрн[а-яёa-z]+ набор[а-яёa-z]*|пемз[а-яёa-z]+|мочалк[а-яёa-z]+|губк[а-яёa-z]+ для (тела|мытья)|ватн[а-яёa-z]+ (диск|палочк)[а-яёa-z]+|(?<![а-яёa-z])вата(?![а-яёa-z])|прокладк[а-яёa-z]+ (гигиен|ежеднев)[а-яёa-z]+|массажн[а-яёa-z]+ (прибор|щетк|щётк|ролик)[а-яёa-z]*|массаж[её]р[а-яёa-z]*|(щ[её]тк|щ[её]точк)[а-яёa-z]+ для (волос|ногтей|ресниц)|брашинг[а-яёa-z]*|термобрашинг[а-яёa-z]*)/iu,
    confidence: 0.75,
    reason:
      "товары личной гигиены (ПП № 656): маркировка производителями/импортёрами с 01.10–01.11.2026, немаркированные остатки — до 30.09.2027",
  },
  {
    wave: "beauty-2025-w3",
    keywords:
