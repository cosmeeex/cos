...
test("щёточки, брашинги и массажёры → волна гигиены 2026", () => {
  assert.equal(classify({ name: "Щеточка для ресниц, в ассортименте, 50шт" }).wave?.id, "hygiene-2026");
  assert.equal(classify({ name: "MelonPro Термобрашинг с керамическим покрытием 25мм" }).wave?.id, "hygiene-2026");
  assert.equal(classify({ name: "ALMASPA Мини-массажер для кожи головы" }).wave?.id, "hygiene-2026");
});

test("чехол для маникюрного набора — не сам набор, маркировке не подлежит", () => {
  assert.equal(classify({ name: "Чехол для маникюрного набора 0151 - 7 предметов" }).wave, null);
});
